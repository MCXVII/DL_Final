...
tower_6 = MaxPooling2D((3,3), strides=(1,1), padding='same')(output1)

output2 = keras.layers.concatenate([tower_4, tower_5, tower_6], axis = 3)

MaxPooling2D((3,3), strides=(1,1), padding='same')(output2)

output2 = Flatten()(output2)
out1 = Dense(5, activation='softmax')(output2)


mod11 = Model(inputs = input_img, outputs = out1)
plot_model(mod11, to_file='mod11.png', show_shapes=True, show_layer_names=True)


checkpoint = ModelCheckpoint(filepath='mod11.h5', monitor='val_accuracy', verbose=1, save_best_only=True, mode='max')
mod11.compile(loss=keras.losses.categorical_crossentropy,
...
his11 = mod11.fit_generator(datagen.flow(x_train1, y_train1, batch_size=32),
                        steps_per_epoch=len(x_train1)/32, epochs=50, validation_data = (x_valid, y_valid), callbacks = [MetricsCheckpoint('logs'), checkpoint])


score = mod11.evaluate(X_test,y_testHot, verbose=0)
