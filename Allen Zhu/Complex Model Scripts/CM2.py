...
mod10 = Model(inputs = input_img, outputs = out1)
plot_model(mod10, to_file='mod10.png', show_shapes=True, show_layer_names=True)


checkpoint = ModelCheckpoint(filepath='mod10.h5', monitor='val_accuracy', verbose=1, save_best_only=True, mode='max')
mod10.compile(loss=keras.losses.categorical_crossentropy,
...
his10 = mod10.fit_generator(datagen.flow(x_train1, y_train1, batch_size=32),
                        steps_per_epoch=len(x_train1)/32, epochs=50, validation_data = (x_valid, y_valid), callbacks = [MetricsCheckpoint('logs'), checkpoint])


score = mod10.evaluate(X_test,y_testHot, verbose=0)
